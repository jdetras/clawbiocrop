...
SKILLS = {
    ...
    "snp-seek": {
        "script": SKILLS_DIR / "snp-seek" / "snp_seek.py",
        "demo_args": ["--demo"],
        "description": "SNP-Seek: IRRI 3K Rice Genomes variant explorer (region/MSU locus → SNPs + varietal-group allele frequencies on IRGSP-1.0/MSU7)",
        "allowed_extra_flags": {"--region", "--locus", "--live"},
        "no_input_required": True,
        "accepts_genotypes": False,
    },
    "rice-pilaf": {
        "script": SKILLS_DIR / "rice-pilaf" / "rice_pilaf.py",
        "demo_args": ["--demo"],
        "description": "RicePilaf: post-GWAS/QTL browser — lift rice GWAS peak intervals to candidate genes with trait-category enrichment",
        "allowed_extra_flags": {"--loci", "--bed"},
        "no_input_required": True,
        "accepts_genotypes": False,
    },
    "crop-ontology": {
        "script": SKILLS_DIR / "crop-ontology" / "crop_ontology.py",
        "demo_args": ["--demo"],
        "description": "Crop & Plant Ontology lookup (Plant Ontology, Trait Ontology, PECO, Crop Ontology via Planteome)",
        "allowed_extra_flags": {"--query", "--term", "--ontology", "--live"},
        "no_input_required": True,
        "accepts_genotypes": False,
    },
    "crop-gwas": {
        "script": SKILLS_DIR / "crop-gwas" / "crop_gwas.py",
        "demo_args": ["--demo"],
        "description": "Crop GWAS: per-SNP association scan with effect size, lambda_GC, and Bonferroni hits for breeding panels",
        "allowed_extra_flags": {"--genotypes", "--phenotype", "--trait"},
        "no_input_required": True,
        "accepts_genotypes": True,
    },
    "crop-genome-annotation": {
        "script": SKILLS_DIR / "crop-genome-annotation" / "crop_genome_annotation.py",
        "demo_args": ["--demo"],
        "description": "Crop genome structural annotation: six-frame ORF finding → gene/CDS models and GFF3",
        "allowed_extra_flags": {"--min-aa"},
        "no_input_required": False,
        "accepts_genotypes": False,
    },
}
